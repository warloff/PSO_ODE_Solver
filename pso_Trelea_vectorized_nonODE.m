...
%   epoch   P(3) - population size, default = 24
%
...
%    pbest - matrix of best values for each particle, traces full flight of
%    swarm. 
%
...
function [OUT,gbestval]=pso_Trelea_vectorized_nonODE(functname,D,varargin)
...
if ( (PSOseed==1) && ~exist('PSOseedValue', 'var') )
    error('PSOseed flag set but no PSOseedValue was input');
end

if exist('PSOseedValue','var')
...
elseif minmax==2
    ...
    [~,idx1] = min((pbestval-ones(size(pbestval))*errgoal).^2);
...
%precalculate inertia weights, if we are not using trelea's vectorized
%functions.
if (trelea==0) %Only compute this if using common pso w/ inertia
    iwt(1)=iw1;
    iwt(2:iwe)=( (iw2-iw1)/(iwe) ) * (2:iwe) + iw1;
    iwt(iwe+1:me)=iw2;
end
...
%Initalize Waitbar:
h=waitbar(0,'1','Name','Optimizing using PSO','CreateCancelBtn','setappdata(gcbf,''canceling'',1)','Position',[.2,.2,.3,.3]);
tic

%Inserted "try/catch" to remove wait-bar on error.
try
...
%         end
        ...
        if plotflg==1
            if (rem(i,df) == 0 ) || (i==me) || (i==1)
                elapsedTime=toc;
                mymsg=[sprintf('GBest = %10.10g Time: ',gbestval),...
                    datestr(elapsedTime/86400,'HH:MM:SS.FFF')];
                waitbar(i/me,h,mymsg,'Position',[.2,.5,.1,.1]);
        ...
        if rstflg == 0
            ...
            elseif minmax == 1
                [tempi,~]        = find(pbestval<=out); % new max pbestvals
            ...
            elseif minmax == 2  % this won't work as it is, fix it later
                ...
                [tempi,~]       = find(sqerr1 <= sqrerr2); % find particles closest to targ
                pbestval(tempi,1) = out(tempi,1); % update pbestvals
                pbest(tempi,:)    = pos(tempi,:); % update pbest positions
        ...
        elseif trelea ==3
            ...
            %Inertia weight was being dynamically calculated, but it can be
            %pre-calculated, which should help with some memory issues. Moved
            %this code to pre-epoch loop.
