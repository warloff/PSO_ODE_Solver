function [ localRMSE ] = Redparticlefitness( varargin ) 
...
% % Uncomment this block of code once to get less strict testing
% % values for getting to RMSE's using an ODE solver.
% % Gbest <0.004, Avgbest<0.07 leaves Avg Best frequently harder to
% % get. Still these are reasonable testing parameters, usually
% % between 3-20min to hit ODE RMSE's.

% if localRMSE(particles+1)<0.004
%    if mean(localRMSE)<0.07
%          RMSE_funct=1;
%          display('switching to ODE RMSE');
%    end
% end



if localRMSE(particles+1)<0.003 %0.0022 is pretty hard.
    if mean(localRMSE)<0.065 % It seems this is actually stricter
        RMSE_funct=1;
        display('switching to ODE RMSE');
    end
end
